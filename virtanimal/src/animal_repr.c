...
static int Animal_Repr_init(Animal_Repr *self, PyObject *args, PyObject *kwds) {
    ...
    static char *kwlist[] = {"id", "thread_count", "energy", "x", "y", "direction", "stop_on_action", "one_thread_per_tick", "maxsteps", NULL};
    ...
    maxsteps = 100;
    stop_on_action = 0;
    one_thread_per_tick = 0;

    if (! PyArg_ParseTupleAndKeywords(args, kwds, "|iiiiii", kwlist, &id, &thread_count, &energy, &x, &y, &direction, &stop_on_action, &one_thread_per_tick, &maxsteps))
        return -1;
    ...
    self->local_subtype = fstANIMAL_DEFAULT;
...
static PyMemberDef Animal_Repr_members[] = 
{
    ...
    {"y", T_INT, offsetof(Animal_Repr, animal.unitvarstruct.y), 0, "int"},
